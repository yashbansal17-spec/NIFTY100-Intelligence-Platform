...
def fetch_live_market_data(force_refresh: bool = False) -> pd.DataFrame:
    ...
    if YFINANCE_AVAILABLE:
        try:
            ...
            logger.info("Fetching live 1-year history from Yahoo Finance for NIFTY 100...")
            data = yf.download(list(ticker_map.keys()), period="1y", interval="1d", group_by="ticker", threads=True, progress=False)
            
            logger.info("Fetching exact today's quotes from Yahoo Finance...")
            today_data = yf.download(list(ticker_map.keys()), period="1d", group_by="ticker", threads=True, progress=False)
            ...
            for ns_symbol, company_id in ticker_map.items():
                try:
                    ...
                    # Merge today's exact quote if available to resolve Yahoo daily NaN quirks
                    try:
                        today_df = today_data[ns_symbol] if len(ticker_map) > 1 else today_data
                        today_df = today_df.dropna(subset=['Close'])
                        if not today_df.empty:
                            df = pd.concat([df[~df.index.isin(today_df.index)], today_df])
                    except Exception as e_today:
                        logger.debug(f"Could not merge today's quote for {company_id}: {e_today}")
    ...
    for _, row in companies_df.iterrows():
        ...
        if company_id in live_records:
            ...
            results.append({
                ...
                "company_name": cname,
                "current_price": round(curr_price, 2),
                "open_price": round(curr_price * (1.0 - (np.random.rand() - 0.5) * 0.02), 2),
                "high_52w": round(high_52w, 2),
                "low_52w": round(low_52w, 2),
